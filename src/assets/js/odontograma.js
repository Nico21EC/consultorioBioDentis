function odo ()  {
    ...
    const itensProcedimento = [{
        ...
        nome: 'Exodoncia Simple Indicada',
        cor: '#ff0000',
        letra: '✘'
    }, {
        nome: 'Sin erupcionar',
        ...
        nome: 'Sellante Indicado',
        cor: '#ff0000',
        letra: 'S'
    }, {
        nome: 'Sellante Presente',
        cor: '#0000ff',
        letra: 'S'
    }, {
        nome: 'Endodoncia Indicada',
        cor: '#ff0000',
        letra: '▲'
    }, {
        nome: 'Endodoncia Realizada',
        cor: '#0000ff',
        letra: '▲'
    }, {
        nome: 'Provisional en Mal Estado',
        cor: '#ff0000',
        letra: 'P'
    }, {
        nome: 'Provisional en Buen Estado',
        cor: '#0000ff',
        letra: 'P'
    }, {
        nome: 'Corona en Mal Estado',
        cor: '#ff0000',
        letra: '◯'
    }, {
        nome: 'Corona en Buen Estado',
        cor: '#0000ff',
        letra: '◯'
    }, {
        nome: 'Procedimiento realizado',
        cor: '#00ff00',
        letra: '✔'
    }, {
        nome: 'Otro',
    ...
    class Procedimento {
        constructor(nome, cor, letra, numeroDente, faceDente, informacoesAdicionais) {
            this.nome = nome;
            this.cor = cor;
            this.letra = letra;
    ...
    const pintarFace = (contexto, procedimento, cor_linha, cor_interior, letra) => {
        ...
        if (procedimento.faceDente === 1 && !dividir) {
            if (contexto) {
                if (procedimento.nome == 'Cariado' || procedimento.nome == 'Obturado') {
                    contexto.beginPath();
                    contexto.moveTo(posicaoX, posicaoY);
                    contexto.lineTo(dimensoesTrapezio.baseMaior + posicaoX, posicaoY);
                    contexto.lineTo(dimensoesTrapezio.baseMenor + posicaoX, dimensoesTrapezio.lateral + posicaoY);
                    ...
                    contexto.fillText(procedimento.letra, (dimensoesTrapezio.baseMenor) / 2 + posicaoX, dimensoesTrapezio.lateral + posicaoY);
                    contexto.stroke();
                }
            }
            ...
            if (contexto) {
                ...
                if (procedimento.nome == 'Cariado' || procedimento.nome == 'Obturado') {
                    prcdms.forEach((procedimentoItem, divisao) => {
                        ...
                        contexto.text = procedimentoItem.letra
                        ...
                        contexto.closePath();
                        contexto.fill();
                        //contexto.fillText(string("procedimento.letra"), dimensoesTrapezio.lateral + posicaoX, dimensoesTrapezio.lateral + posicaoY);
                        contexto.stroke();
                    })
                } else {
                    prcdms.forEach((procedimentoItem, divisao) => {
                        ...
                        contexto.closePath();
                        //contexto.fill();
                        contexto.fillText(procedimento.letra, dimensoesTrapezio.lateral + posicaoX, dimensoesTrapezio.lateral + posicaoY);
                        contexto.stroke();
                    })
                }
        ...
        if (procedimento.faceDente === 2 && !dividir) {
            if (contexto) {
                if (procedimento.nome == "Cariado" || procedimento.nome == "Obturado") {
                    ...
                    contexto.closePath();
                    contexto.font = "15pt Verdana";
                    contexto.fillText(procedimento.letra, (dimensoesTrapezio.lateral + posicaoX) + 38, (dimensoesTrapezio.baseMaior) / 2 + posicaoY + 8);
                    contexto.stroke();
                }
            }
        ...
        if (procedimento.faceDente === 3 && !dividir) {
            if (contexto) {
                if (procedimento.nome == 'Cariado' || procedimento.nome == 'Obturado') {
                    ...
                    contexto.closePath();
                    contexto.font = "15pt Verdana";
                    contexto.fillText(procedimento.letra, (dimensoesTrapezio.baseMenor) / 2 + posicaoX, dimensoesTrapezio.baseMaior + posicaoY - 2.8);
                    contexto.stroke();
                }
            ...
            if (contexto) {
                ...
                if (procedimento.nome == 'Cariado' || procedimento.nome == 'Obturado') {
                    ...
                    prcdms.forEach((procedimentoItem, divisao) => {
                        ...
                        contexto.closePath();
                        contexto.font = "15pt Verdana";
                        contexto.fillText(procedimento.letra, (dimensoesTrapezio.baseMenor) / 2 + posicaoX, dimensoesTrapezio.baseMaior + posicaoY);
                        contexto.stroke();
                    })
                }
        ...
        if (procedimento.faceDente === 4 && !dividir) {
            if (contexto) {
                if (procedimento.nome == 'Cariado' || procedimento.nome == 'Obturado') {
                    ...
                    contexto.closePath();
                    contexto.font = "15pt Verdana";
                    contexto.fillText(procedimento.letra, (dimensoesTrapezio.baseMaior + posicaoX) + 13, (dimensoesTrapezio.lateral) / 2 + posicaoY + 36);
                    contexto.stroke();
                }
            }
        ...
        if (procedimento.faceDente === 5 && !dividir) {
            if (contexto) {
                if (procedimento.nome == 'Cariado' || procedimento.nome == 'Obturado') {
                    ...
                    contexto.closePath();
                    contexto.font = "15pt Verdana";
                    contexto.fillText(procedimento.letra, (dimensoesTrapezio.baseMenor) / 2 + posicaoX, (dimensoesTrapezio.baseMaior) / 2 + posicaoY + 6);
                    contexto.stroke();
                }
    ...
    const resizeCanvas = () => {
        if (window.innerWidth >= 800) {
            ...
            alert("PANTALLA MUY PEQUEÑA! Accede al odontograma a través de un dispositivo con una pantalla más grande!")
        ...
        //base_image.src = 'assets/img/18.png';
    ...
    const exibeMarcacoes = () => {
        procedimentos.forEach(element => {
            pintarFace(contexto2, element, 'black', element.cor, element.letra)
        });
    }
    ...
    const iniciaOdontograma = () => {
        ...
        document.querySelector("#nomeProcedimento").addEventListener('change', (event) => {
            ...
            if (procedimento.value !== '') {
                ...
                document.querySelector("#letra").value = procedimento.letra
                document.getElementById("letra").style.color = procedimento.cor
        ...
        document.querySelector('#saveBtn').onclick = () => {
            saveOdonto()
            toastr.success("Odontograma Guardado Exitosamente");
        }
